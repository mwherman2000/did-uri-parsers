...
public class Parser
{
    ...
    static public void Main(String[] args)
    {
        ...
        if (ok)
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                else if (args[i].Equals("-testcases"))
                    arguments["TestCases"] = args[++i];
        ...
        if (ok)
        {
            if (!arguments.ContainsKey("File") &&
                !arguments.ContainsKey("TestCases") &&
                !arguments.ContainsKey("String"))
            {
                error = "insufficient arguments: -file, -testcases or -string required";
                ok = false;
            }
        }
        ...
        if (!ok)
        {
            ...
            Console.WriteLine("usage: Parser [-rule rulename] [-trace] <-file file | -testcases file | -string string> [-visitor visitor]");
        }
        else
        {
            try
            {
                ...
                if (arguments.ContainsKey("TestCases"))
                {
                    string line;
                    int counter = 0;
                    System.IO.StreamReader file = new System.IO.StreamReader(arguments["TestCases"]);
  ...
  static private Rule Parse(String rulename, String text, bool trace)
  {
    ...
    else if (rulename.ToLower().Equals("transformer-quotedvalue".ToLower())) rule = Rule_transformer_quotedvalue.Parse(context);
    else if (rulename.ToLower().Equals("transformer-numericvalue".ToLower())) rule = Rule_transformer_numericvalue.Parse(context);
    else if (rulename.ToLower().Equals("transformer-option".ToLower())) rule = Rule_transformer_option.Parse(context);
    else if (rulename.ToLower().Equals("transformer-stringvalue".ToLower())) rule = Rule_transformer_stringvalue.Parse(context);
    else if (rulename.ToLower().Equals("transformer-char".ToLower())) rule = Rule_transformer_char.Parse(context);
    else if (rulename.ToLower().Equals("transform-char".ToLower())) rule = Rule_transform_char.Parse(context);
