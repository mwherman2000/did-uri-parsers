...
sealed public class Rule_SP:Rule
{
  private Rule_SP(String spelling, List<Rule> rules) :
  base(spelling, rules)
  {
  }
  ...
  public static Rule_SP Parse(ParserContext context)
  {
    ...
    if (parsed)
    {
        rule = new Rule_SP(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
    }
    else
    {
        context.index = s0;
