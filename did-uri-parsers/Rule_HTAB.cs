...
sealed public class Rule_HTAB:Rule
{
  private Rule_HTAB(String spelling, List<Rule> rules) :
  base(spelling, rules)
  {
  }
  ...
  public static Rule_HTAB Parse(ParserContext context)
  {
    ...
    if (parsed)
    {
        rule = new Rule_HTAB(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
    }
    else
    {
        context.index = s0;
