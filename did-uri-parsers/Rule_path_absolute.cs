...
sealed public class Rule_path_absolute:Rule
{
  private Rule_path_absolute(String spelling, List<Rule> rules) :
  base(spelling, rules)
  {
  }
  ...
  public static Rule_path_absolute Parse(ParserContext context)
  {
    ...
    if (parsed)
    {
        rule = new Rule_path_absolute(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
    }
    else
    {
        context.index = s0;
