...
sealed public class Rule_authority:Rule
{
  private Rule_authority(String spelling, List<Rule> rules) :
  base(spelling, rules)
  {
  }
  ...
  public static Rule_authority Parse(ParserContext context)
  {
    ...
    if (parsed)
    {
        rule = new Rule_authority(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
    }
    else
    {
        context.index = s0;
    ...
    context.Pop("authority", parsed);

    return (Rule_authority)rule;
  }
}
