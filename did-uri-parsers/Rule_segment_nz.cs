...
sealed public class Rule_segment_nz:Rule
{
  private Rule_segment_nz(String spelling, List<Rule> rules) :
  base(spelling, rules)
  {
  }
  ...
  public static Rule_segment_nz Parse(ParserContext context)
  {
    ...
    if (parsed)
    {
        rule = new Rule_segment_nz(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
    }
    else
    {
        context.index = s0;
