...
sealed public class Rule_URI:Rule
{
  private Rule_URI(String spelling, List<Rule> rules) :
  base(spelling, rules)
  {
  }
  ...
  public static Rule_URI Parse(ParserContext context)
  {
    ...
    if (parsed)
    {
        rule = new Rule_URI(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
    }
    else
    {
        context.index = s0;
