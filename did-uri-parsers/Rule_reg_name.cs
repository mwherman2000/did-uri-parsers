...
sealed public class Rule_reg_name:Rule
{
  private Rule_reg_name(String spelling, List<Rule> rules) :
  base(spelling, rules)
  {
  }
  ...
  public static Rule_reg_name Parse(ParserContext context)
  {
    ...
    if (parsed)
    {
        rule = new Rule_reg_name(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
    }
    else
    {
        context.index = s0;
    ...
    context.Pop("reg-name", parsed);

    return (Rule_reg_name)rule;
  }
}
