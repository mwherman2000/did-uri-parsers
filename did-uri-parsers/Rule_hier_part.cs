...
sealed public class Rule_hier_part:Rule
{
  private Rule_hier_part(String spelling, List<Rule> rules) :
  base(spelling, rules)
  {
  }
  ...
  public static Rule_hier_part Parse(ParserContext context)
  {
    ...
    if (parsed)
    {
        rule = new Rule_hier_part(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
    }
    else
    {
        context.index = s0;
    ...
    context.Pop("hier-part", parsed);

    return (Rule_hier_part)rule;
  }
}
