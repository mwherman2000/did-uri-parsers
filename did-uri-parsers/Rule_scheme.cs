...
sealed public class Rule_scheme:Rule
{
  private Rule_scheme(String spelling, List<Rule> rules) :
  base(spelling, rules)
  {
  }
  ...
  public static Rule_scheme Parse(ParserContext context)
  {
    ...
    if (parsed)
    {
        rule = new Rule_scheme(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
    }
    else
    {
        context.index = s0;
