...
sealed public class Rule_host:Rule
{
  private Rule_host(String spelling, List<Rule> rules) :
  base(spelling, rules)
  {
  }
  ...
  public static Rule_host Parse(ParserContext context)
  {
    context.Push("host");
    ...
    if (parsed)
    {
        rule = new Rule_host(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
    }
    else
    {
        context.index = s0;
