...
sealed public class Rule_sub_delims:Rule
{
  private Rule_sub_delims(String spelling, List<Rule> rules) :
  base(spelling, rules)
  {
  }
  ...
  public static Rule_sub_delims Parse(ParserContext context)
  {
    ...
    if (parsed)
    {
        rule = new Rule_sub_delims(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
    }
    else
    {
        context.index = s0;
