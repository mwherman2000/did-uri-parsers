...
sealed public class Rule_did_uri:Rule
{
  private Rule_did_uri(String spelling, List<Rule> rules) :
  base(spelling, rules)
  {
  }
  ...
  public static Rule_did_uri Parse(ParserContext context)
  {
    context.Push("did-uri");
    ...
    if (parsed)
    {
        rule = new Rule_did_uri(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
    }
    else
    {
        context.index = s0;
    ...
    context.Pop("did-uri", parsed);

    return (Rule_did_uri)rule;
  }
}
